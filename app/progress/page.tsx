...
export default function ProgressPage() {
  ...
  const getXpProgress = (xp: number) => {
    ...
    if (xp >= 10000) return { current: 10000, next: 10000, progress: 100 };
    return {
      current,
      next,
    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <StatsCard icon="⭐" label="Level"    value={`${levelInfo.level}`}                          subtitle={levelInfo.label} />
          <StatsCard icon="💎" label="Total XP" value={`${profile?.total_xp || 0}`}                  subtitle="Experience Points" />
          <StatsCard icon="🔥" label="Streak"   value={`${profile?.current_streak_days || 0} days`}  subtitle={`Longest: ${profile?.longest_streak_days || 0} days`} />
          <StatsCard icon="🎯" label="Sessions" value={`${profile?.total_sessions_completed || 0}`}  subtitle={`${profile?.total_messages_sent || 0} messages`} />
        </div>
